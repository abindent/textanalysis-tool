...
export namespace Tools {
  ...
  export class Analyser {
    ...
    private async extractKeywords(): Promise<void> {
      ...
      const topN =
        typeof options === "object" && options.topN ? options.topN : 5;

      this.keywords = this.keywordExtractor.extractKeywords(
        this.raw_text,
        topN,
      );
    ...
    private async calculateReadability(): Promise<void> {
      ...
      this.logOperation(
        "Calculated readability Metrics (Flesch-Kincaid & SMOG)",
      );
    }
    ...
    private async compareTexts(): Promise<void> {
      ...
      const comparisonResult = this.textDiff.compare(
        this.raw_text,
        compareWith,
      );
